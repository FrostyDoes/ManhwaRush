...
async function handleCheckoutSessionCompleted(
  ...
  if (session.metadata?.type === "coin_purchase" && session.metadata?.coins) {
    ...
    if (!userId || !coins) {
      console.error("Missing user ID or coins amount in metadata");
      return new Response(
        JSON.stringify({ error: "Missing user ID or coins amount" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    try {
      // Call the update-user-coins function
      const updateCoinsResponse = await fetch(
        `${Deno.env.get("SUPABASE_URL")}/functions/v1/update-user-coins`,
        {
          method: "POST",
          headers: {
      ...
      console.log("Update coins result:", updateCoinsResult);

      if (!updateCoinsResult.success) {
        throw new Error(`Failed to update coins: ${updateCoinsResult.error}`);
      }

      return new Response(
        JSON.stringify({
          message: "Coin purchase processed successfully",
          coins,
          userId,
        }),
        {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to process coin purchase" }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
  ...
  // Continue with subscription handling
